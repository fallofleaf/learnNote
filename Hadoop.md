...
id --> id1(理论)
	id1 --> Yarn基础架构
	id1 --> Yarn工作机制
	id1 --> Mapreduce/HDFS/Yarn
	id1 --> id11(调度器和调度算法)
		id11 --> FIFO
		id11 --> 容量调度器
		id11 --> 公平调度器
	id1 --> yarn在生产环境下要配置哪些参数
	id1 --> 命令行操作Yarn
id --> id2(怎么用) 
	id2 --> 生产环境下参数怎么配置
	id2 --> 容量调度器在生产环境配置
...
​	Yarn是一个资源调度平台，负责为运算程序提供程序服务器运算资源，相当于一个分布式的操作系统平台，而MapReduce等运算程序则相当于运行于操作系统之上的应用程序

#### Yarn基础架构

![image-20211115135158883](Hadoop.assets/image-20211115135158883.png)

#### Yarn工作机制

![image-20211115135237820](Hadoop.assets/image-20211115135237820.png)
...
![image-20211115135842214](Hadoop.assets/image-20211115135842214.png)

##### 作业提交过程之YARN

![image-20211115135927210](Hadoop.assets/image-20211115135927210.png)

##### 作业提交过程之HDFS&MapReduce

![image-20211115135959468](Hadoop.assets/image-20211115135959468.png)
...
6. 作业完成
   ...
   处理向应用管理器请求作业进度外，客户端每5s都会通过调用waitForCompletion()来检查作业是否完成，时间间隔可以通过mapreduce.client.completion.pollinterval来设置，作业完成以后，应用管理器和Container会请求工作状态，作业的信息会被作业历史服务器存储以备之后用户检查

#### Yarn调度器和调度算法

​	目前，Hadoop调度器主要有三种：FIFO、容量(Capacity Scheduler)和公平(Fair Scheduler)。Apache Hadoop3.1.3默认的资源调度器是Capacity Scheduler
...
![image-20211115142535775](Hadoop.assets/image-20211115142535775.png)
...
![image-20211115142659232](Hadoop.assets/image-20211115142659232.png)
...
4. 多租户：
   ...
   为了防止同一个用户的作业独占队列中的资源，该调度器会对统一用户提交的作业所占资源量进行限定

容量调度器资源分配算法

![image-20211115143046994](Hadoop.assets/image-20211115143046994.png)
...
3. 容器资源分配
   ...
   1. 任务和数据在同一节点
   2. 任务和数据在同一机架
   3. 任务和数据不再同一节点也不在同一机架

##### 公平调度器(Fair Scheduler)

Fair Scheduler是Facebook开发的多用户调度器

![image-20211115143727439](Hadoop.assets/image-20211115143727439.png)
...
![image-20211115144243805](Hadoop.assets/image-20211115144243805.png)

- 公平调度器的设计目标是：在时间尺度上，所有做作业获得公平的资源，某一时刻一个作业应获取资源和实际获取资源的差距叫“缺额”
- 调度器会优先为缺额大的作业分配资源
...
2. Fair策略
   ...
   Fair策略(默认)是一种基于最大最小公平算法实现的资源多路复用方式，默认情况下，每个队列内部采用该方式分配资源，这意味着，如果一个队列中有两个应用程序同时运行，则每个应用程序可得到1/2的资源，如果三个应用程序同时运行，则每个应用程序可以得到1/3的资源
...
![image-20211115144904070](Hadoop.assets/image-20211115144904070.png)

![image-20211115145002053](Hadoop.assets/image-20211115145002053.png)

公平调度器资源分配算法

![image-20211115145056374](Hadoop.assets/image-20211115145056374.png)

1.队列资源分配

需求：集群总资源100，有三个队列，对资源的需求分别是queueA -> 20，queueB ->50，queueC -> 30

第一次算：100/3=33.3

queueA：分33.33->多13.33

queueB：分33.33->多16.67
...
Yarn状态的查询，除了可以在hadoop103:8080页面查看外，还可以通过命令行操作，常见操作命令如下：

```bash
[zxk@hadoop102 ~]$ myhadoop.sh start
=================启动hadoop集群==================
------------启动hdfs-----------------
...
//运行wordcount案例
[zxk@hadoop102 ~]$ cd /opt/module/hadoop-3.3.1/
[zxk@hadoop102 hadoop-3.3.1]$ hadoop jar share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar wordcount /input /output1
```
...
```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn application -list
2021-11-16 13:46:12,826 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Total number of applications (application-types: [], states: [SUBMITTED, ACCEPTED, RUNNING] and tags: []):0
                Application-Id      Application-Name        Application-Type          User           Queue                   State        Final-State             Progress                        Tracking-URL
```

根据Application状态过滤 yarn application -list -appStates(所有状态ALL、NEW、NEW_SAVING、SUBMITTED、ACCEPTED、RUNNING、FINISHED、FAILED、KILLED)

```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn application -list -appStates FINISHED
2021-11-16 13:50:37,401 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.                                                   10.103:8032
Total number of applications (application-types: [], states: [FINISHED] and tags: []):1
                Application-Id      Application-Name        Application-Type          User           Queue                                                                      State        Final-State             Progress                        Tracking-URL
application_1637041159575_0001            word count               MAPREDUCE           zxk         default                F                                                   INISHED          SUCCEEDED                 100% http://hadoop102:19888/jobhistory/job/job_1637041159575_0001
[zxk@hadoop102 hadoop-3.3.1]$
```
...
查询Application日志 yarn logs -appliactionId 应用id

```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn logs -applicationId application_1637041159575_0001
```

查询Container日志 yarn logs -applicationId 程序ID -containerId 容器ID

```bash
[zxk@hadoop102 
hadoop-3.3.1]$  yarn logs -applicationId application_1637041159575_0001  -containerId  container_1637041159575_0001_01_000001
```

##### yarn applicatitonattempt 查看尝试运行的任务

列出所有Appliaction尝试的列表

```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn applicationattempt -list application_1637041159575_0001
2021-11-16 14:01:31,969 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Total number of application attempts :1
         ApplicationAttempt-Id                 State                        AM-Container-Id                            Tracking-URL
appattempt_1637041159575_0001_000001                FINISHED    container_1637041159575_0001_01_000001  http://hadoop103:8088/proxy/application_1637041159575_0001/
```

打印ApplicationAttemp状态

```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn applicationattempt -status appattempt_1637041159575_0001_000001
2021-11-16 14:07:05,739 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Application Attempt Report :
        ApplicationAttempt-Id : appattempt_1637041159575_0001_000001
        State : FINISHED
        AMContainer : container_1637041159575_0001_01_000001
        Tracking-URL : http://hadoop103:8088/proxy/application_1637041159575_0001/
        RPC Port : 42599
        AM Host : hadoop102
        Diagnostics :
```
...
列出所有container  yarn container -list appattemptId

```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn container -list appattempt_1637041159575_0001_000001
2021-11-16 14:08:43,398 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
...
```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn container -status container_1637041159575_0001_01_000001
2021-11-16 14:10:58,246 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Container with id 'container_1637041159575_0001_01_000001' doesn't exist in RM or Timeline Server.
```
...
```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn node -list -all
2021-11-16 14:12:15,225 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Total Nodes:3
         Node-Id             Node-State Node-Http-Address       Number-of-Running-Containers
 hadoop103:37107                RUNNING    hadoop103:8042                                  0
 hadoop102:45703                RUNNING    hadoop102:8042                                  0
 hadoop104:36531                RUNNING    hadoop104:8042                                  0
```
...
```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn rmadmin -refreshQueues
```
...
```bash
[zxk@hadoop102 hadoop-3.3.1]$ yarn queue -status default
2021-11-16 14:14:40,583 INFO client.DefaultNoHARMFailoverProxyProvider: Connecting to ResourceManager at hadoop103/192.168.10.103:8032
Queue Information :
Queue Name : default
        State : RUNNING
        Capacity : 100.00%
...
2. NodeManager相关
   ...
   yarn.nodemanager.resource.pcores-vcores-multiplier 虚拟核数和物理核数乘数，例如：4核8线程，该参数就应该设为2，默认1.0
   ...
   yarn.nodemanager.vmem-pmem-ratio 虚拟物理内存比例，默认2:1
...
3. Container相关
   ...
   yarn.scheduler.maximun-allocation-vcores 容器最大CPU核数，默认4个

![image-20211118141158931](Hadoop.assets/image-20211118141158931.png)

### 2. Yarn案例实操

进行下面的操作前，先把集群的每一台机器都配置快照，这样可以很方便的回到初始状态
...
3. 修改yarn-site.xml配置参数如下
   ...
   ![image-20211119134306263](Hadoop.assets/image-20211119134306263.png)
...
6. 执行WordCount程序
   ...
   ```bash
   hadoop102 hadoop jar
   share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar wordcount
   /input /output
   ```
...
2. 创建多队列的好处
   ...
   1. 防止递归死循环代码，把所有资源全部耗尽

   2. 实现任务的降级，特殊时期保证重要的任务队列资源充足

      业务部门1(重要)>业务部门2(比较重要)>下单(一般)>购物车(一般)>登陆注册(次要)
...
1. 在capacity-scheduler.xm中配置如下：
   ...
   4. 重启yarn，或者执行yarn rmadmin -refreshQueues刷新队列，就可以看到两条队列
...
1. hadoop jar的方式
   ...
   ```bash
   hadoop102 hadoop jar
   share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar wordcount -D
...
2. 打jar包的方式
   ...
   这样任务在集群提交时，就会提交到hive队列

##### 任务优先级

容量调度器，支持任务优先级的配置，在资源紧张时，优先级高的任务将优先获取资源。默认情况，Yarn将所有任务的优先级限制为0，若想使用任务的优先级功能，需要开放该限制

1. 修改yarn-site.xml文件，添加以下参数
...
2. 分发配置，并重启yarn
   ...
   ```bash
   hadoop102 xsync yarn-site.xml
   hadoop103 sbin/stop-yarn.sh
   hadoop103 sbin/start-yarn.sh
   ```

3. 模拟资源紧张环境，可以联系提交下面的任务，指导新提交的任务申请不到资源为止

   ```bash
   hadoop102 hadoop jar /opt/module/hadoop3.3.1/share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar pi 5
   2000000
   ```
...
4. 再次重新提交优先级高的任务
   ...
   ```bash
   hadoop102  hadoop jar /opt/module/hadoop3.3.1/share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar pi -D
   mapreduce.job.priority=5 5 2000000
   ```

5. 也可以通过下面的命令修改正在执行的任务的优先级

   yarn application -appID  -updatePriority 优先级

   ```bash
   hadoop102 yarn application -appID
...
创建两个队列，分别是test和zxk(以用户所属组命令)。实现以下效果：若用户提交任务时指定队列，则任务提交到指定队列运行；若未指定队列，test用户提交的任务到root.group.test队列运行，zxk提交的任务到root.group.zxk队列运行，group为用户所属组
...
2. 配置fair-scheduler.xml(注意用户名和组根据实际进行配置)
...
3. 分发配置，重启yarn
   ...
   ```bash
   hadoop102 $ xsync yarn-site.xml
   hadoop102 $ xsync fair-scheduler.xml
   hadoop103 $ sbin/stop-yarn.sh
   hadoop103 $ sbin/start-yarn.sh
   ```
...
1. 提交任务时指定队列，按照配置规则，任务会到指定的root.test队列
   ...
   ```bash
   hadoop102 $ hadoop jar /opt/module/hadoop3.3.1/share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar pi -
   Dmapreduce.job.queuename=root.test 1 1
   ```

2. 提交任务时不指定队列，按照配置规则，任务会到root.zxk.zxk队列

   ```bash
   hadoop102 $ hadoop jar /opt/module/hadoop3.3.1/share/hadoop/mapreduce/hadoop-mapreduce-examples-3.3.1.jar pi 1 1
   ```

#### Yarn的Tool接口案例

1. 需求：实现命令行可以动态传递参数，编写Yarn的tool接口
...
2. 具体步骤
   ...
   5. 在HDFS上准备输入文件，假设为/input目录，向集群提交该jar包
      ...
      ```bash
      hadoop102 yarn jar YarnDemo.jar
      com.atguigu.yarn.WordCountDriver wordcount /input /output
      ```

注意此时提交的3个参数，第一个用于生产特定的tool，第二个和第三个为输入输出目录，此时如果我们希望加入设置参数，可以在wordcount后面添加参数，如
